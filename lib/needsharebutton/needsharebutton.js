  ...
  window.needShareButton = function (elem, options) {
    ...
    root.share = {
      ...
      'wechat': function (el) {
        ...
        //更改微信图片展示
        var box = document.querySelector('#needsharebutton-float');
        var img = box.getElementsByClassName('need-share-wechat-code-image')[0];
        if (img) {
          img.remove();
        } else {
    ...
    window.onload = function (ev) {
      ...
      if (NexT.utils.isMobile()) {
        // close on click outside 注释点外部关闭
        document.addEventListener('click', function (event) {
          ...
          if (!closest(event.target, '.need-share-button-opened')) {
            if (openedEl) {
              ...
              // var el = closest(document.querySelector(".fa fa-share-alt"), root.elem);
              if (el) {
        ...
        document.querySelector("#needsharebutton-float .btn").style.visibility = "initial";
        document.querySelector("#needsharebutton-float .btn").style.marginLeft = "-8px";
      } else {
        //页面加载完弹出分享
        var el = closest(document.getElementById("needsharebutton-float"), root.elem);
        // var el = closest(document.querySelector(".fa fa-share-alt"), root.elem);
        if (el) {
          if (!el.classList.contains('need-share-button-opened')) {
            createDropdown(el);
